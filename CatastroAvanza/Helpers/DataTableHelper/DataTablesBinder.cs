...
namespace CatastroAvanza.Helpers.DataTableHelper
{
    public class DataTablesBinder : IModelBinder
    {
        ...
        /// You should override this method to provide a custom type for internal binding to process.
        /// </summary>
        ...
        protected virtual NameValueCollection ResolveNameValueCollection(HttpRequestBase request)
        {
            ...
            {
                throw new ArgumentException($"The provided HTTP method ({request.HttpMethod}) is not a valid method to use with DataTablesBinder. Please, use HTTP GET or POST methods only.", "method");
            }
        }
        ...
        protected virtual List<Column> GetColumns(NameValueCollection collection)
        {
            try
            {
                ...
                for (int i = 0; i < collection.Count; i++)
                {
                    ...
                    {
                        break; // Stops iterating because there's no more columns.
                    }
                }
